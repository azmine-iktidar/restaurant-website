...
const Switcher = memo(() => {
  ...
  useEffect(() => {
    ...
    if (layout) {
      ...
      document.body.setAttribute("data-layout", Values.layout);
      setSwitcherActive((prev) => {
        return [
    ...
    if (
      ...
      document.body.setAttribute("data-body-bg", bgColor.backgroundColor);
    }
    setSwitcherActive((prev) => {
      return [
        {
              ...
              <ul className="pattern-switcher theme-panel-save">
                {BgSmallImgArr.map(({ ptSmall, ptLarge }, ind) => (
                  <li
                    ...
                    key={ind}
                  >
                    <img src={ptSmall} alt={"small" + (ind + 1)}
                      onClick={() => {
